...
### 关于测试
在P3~P6阶段所有测试均可正常与Mars对拍，对于P7测试有如下几点提示

- 与Mars对拍： 由于Mars模拟的是单周期CPU且不具备外部中断信号功能，所以请**谨慎与Mars对拍，辩证地看待测评结果**，与Mars对拍时中断信号只涉及使用timer的0模式，但是依旧存在上述问题
- 与同学对拍：
   - 中断形式能够涉及所有的内容（外部中断请移步**int test**，由于效率原因，带tb的外部中断测试并不会出现在**random test**生成的测试样例中）
   - 由于各个CPU的阻塞转发不尽相同，测评机**未设置一定的宽容时限**，同样可能导致使用timer中断时同学CPU对拍结果的不一致
- 关于数据
   - 数据生成为本人在大二完成P7时同步编写，数据构造思路有个人倾向，**不一定**能够做到尽善尽美，请各位理性、辩证地看待
   - 同时，如果有更好的数据构造方案，欢迎各位分享思路或者自行开发后提交pr🙂

### 关于维护
如在使用cot时遇到相关问题，请检查是否在REAMDE中有所提及。若还有疑问可在Issue区发布 :)

### 关于彩蛋
通关P7后，在**非lazy**模式下运行cot，在选择页面输入`cscore`可触发彩蛋！
